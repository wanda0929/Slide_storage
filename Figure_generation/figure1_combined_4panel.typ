...
#let ball(pos, kind, label, label-color: white, size: 0.38, sub: none, sup: none) = {
  ...
  if sub != none {
    ...
    content((rel: (size * 0.72 - 0.2, size * 0.62 - 0.9), to: pos), text(fill: red, weight: "bold", size: 8pt)[state #sup])
  } else {
    content(pos, text(fill: label-color, weight: "bold", size: 8pt)[#label])
  }
}
...
#figure(
  grid(
    ...
    // ══════════════════════════════════════════════════════════════
    //  PANEL (a): 3D Rydberg lattice  — VERBATIM from original
    // ══════════════════════════════════════════════════════════════
    ...
    //  TITLE
    // ══════════════════════════════════════════
    content((-8.0, 6.3), text(weight: "bold", size: 15pt)[Lattice structure])
    content((-10.5, 6.3), anchor: "west",
    ...
    // ══════════════════════════════════════════
    // Decayed palette for PBC boundary copies (rightmost col j=nv-1, bottommost row i=0)
    ...
    // ══════════════════════════════════════════
    // PBC colours (used by torus rings)
    let pbc-tb = rgb("#9933cc")   // purple
    let pbc-lr = rgb("#dd7700")   // orange
    ...
    content((g-sx - 0.2, g-sy - 1.7), anchor: "south-west",
      angle: 0deg,
    ...
    // ══════════════════════════════════════════
    //  3D TORUS VISUALIZATION  (depth-sorted quads)
    // ══════════════════════════════════════════
    let tcx = 5.1
    let tcy = 1.6
    let tR  = 4.2       // major radius (in internal coords, scaled down)
    let tr  = 1.8       // tube radius
    let t-scale = 0.35  // global scale factor to fit the inset
    let v-max = 360     // toroidal sweep (full circle)

    // Viewing direction & projection (matched to Janosh reference)
    let vdx = 2.7
    let vdy = 1.0
    let vdz = -2.0
    let upx = 0.0
    ...
    let vlen = calc.sqrt(vdx*vdx + vdy*vdy + vdz*vdz)
    let fwx = vdx/vlen
    let fwy = vdy/vlen
    let fwz = vdz/vlen
    // right = forward × up
    let rx0 = fwy*upz - fwz*upy
    let ry0 = fwz*upx - fwx*upz
    let rz0 = fwx*upy - fwy*upx
    let rlen = calc.sqrt(rx0*rx0 + ry0*ry0 + rz0*rz0)
    let rrx = rx0/rlen
    let rry = ry0/rlen
    let rrz = rz0/rlen
    // true up = right × forward
    let tux = rry*fwz - rrz*fwy
    let tuy = rrz*fwx - rrx*fwz
    let tuz = rrx*fwy - rry*fwx

    // Additional rotation around z (146 deg to match Janosh aesthetic)
    let rot-ang = 146.0 * calc.pi / 180.0
    let cra = calc.cos(rot-ang)
    let sra = calc.sin(rot-ang)

    // Torus parametric point
    let torus-pt(u-deg, v-deg) = {
      let u = u-deg * calc.pi / 180.0
      let v = v-deg * calc.pi / 180.0
      let rad = tR + tr * calc.cos(u)
      let x3 = rad * calc.cos(v)
      let y3 = rad * calc.sin(v)
      let z3 = tr * calc.sin(u)
      // Rotate around z
      let xr = x3 * cra - y3 * sra
      let yr = x3 * sra + y3 * cra
      let zr = z3
      // Project: dot with right & up
      let px = rrx*xr + rry*yr + rrz*zr
      let py = tux*xr + tuy*yr + tuz*zr
      (tcx + px * t-scale, tcy + py * t-scale)
    }

    // Depth for sorting (dot with forward)
    let torus-depth(u-deg, v-deg) = {
      let u = u-deg * calc.pi / 180.0
      let v = v-deg * calc.pi / 180.0
      let rad = tR + tr * calc.cos(u)
      let x3 = rad * calc.cos(v)
      ...
      let yr = x3 * sra + y3 * cra
      let zr = z3
      fwx*xr + fwy*yr + fwz*zr
    }

    // Normal for shading (dot with a light direction)
    let torus-normal-light(u-deg, v-deg) = {
      let u = u-deg * calc.pi / 180.0
      let v = v-deg * calc.pi / 180.0
      // Outward normal of torus
      let nx = calc.cos(u) * calc.cos(v)
      let ny = calc.cos(u) * calc.sin(v)
      let nz = calc.sin(u)
      // Rotate
      let nxr = nx * cra - ny * sra
      let nyr = nx * sra + ny * cra
      let nzr = nz
      // Light from upper-right-front
      let lx = 0.4
      let ly = -0.5
      let lz = 0.75
      let ll = calc.sqrt(lx*lx + ly*ly + lz*lz)
      (nxr*lx + nyr*ly + nzr*lz) / ll
    }

    // Build depth-sorted quads
    let nu-steps = 32
    let nv-steps = 40
    let u-step = 360.0 / nu-steps
    let v-step = v-max / nv-steps

    let quads = ()
    for u-idx in range(nu-steps) {
      for v-idx in range(nv-steps) {
        let u = u-idx * u-step
        let v = v-idx * v-step
        let u-next = calc.rem(u-idx + 1, nu-steps) * u-step
        let v-next = if v-idx < nv-steps - 1 { v + v-step } else { v-max }
        let p1 = torus-pt(u, v)
        let p2 = torus-pt(u-next, v)
        let p3 = torus-pt(u-next, v-next)
        let p4 = torus-pt(u, v-next)
        // Depth = average of 4 corners
        let d1 = torus-depth(u, v)
        let d2 = torus-depth(u-next, v)
        let d3 = torus-depth(u-next, v-next)
        let d4 = torus-depth(u, v-next)
        let davg = (d1 + d2 + d3 + d4) / 4.0
        // Shading from normal
        let um = u + u-step / 2.0
        let vm = v + v-step / 2.0
        let nlight = torus-normal-light(um, vm)
        quads.push((depth: davg, p1: p1, p2: p2, p3: p3, p4: p4, shade: nlight))
      }
    }

    // Draw quads (painter's algorithm: far first)
    for quad in quads.sorted(key: q => -q.depth) {
      // Map shade [-1,1] to blue palette
      let s = (quad.shade + 1.0) / 2.0   // 0..1
      let r-val = int(calc.round(12 + s * (200 - 12)))
      let g-val = int(calc.round(40 + s * (226 - 40)))
      let b-val = int(calc.round(72 + s * (252 - 72)))
      let face-col = rgb(
        calc.min(255, calc.max(0, r-val)),
        calc.min(255, calc.max(0, g-val)),
        calc.min(255, calc.max(0, b-val)))
      line(quad.p1, quad.p2, quad.p3, quad.p4, close: true,
        fill: face-col, stroke: 0.2pt + rgb("#14365a40"))
    }

    // --- PBC rings (front/back split for clarity) ---
    // Collect contiguous segments, then draw back segments first, front on top.

    // Generic ring segmenter: takes a list of (pt, is-front) pairs,
    // returns (back-segments, front-segments) where each segment is a point list.
    // A segment boundary occurs when is-front changes.

    // ── Orange toroidal ring (u=0, sweep v 0..360) ──
    {
      let n = 180
      let back-segs = ()
      let front-segs = ()
      ...
      let cur-front = none
      for k in range(n + 1) {
        let v = float(k) / float(n) * 360.0
        let pt = torus-pt(0.0, v)
        let d = torus-depth(0.0, v)
        let is-front = d < 0
        if cur-front == none { cur-front = is-front }
        if is-front != cur-front {
          // boundary: push current segment, start new one with shared point
          if cur-pts.len() > 1 {
            if cur-front { front-segs.push(cur-pts) } else { back-segs.push(cur-pts) }
          }
          cur-pts = (pt,)
          cur-front = is-front
        } else {
      ...
      // Draw back segments first (thin, faded)
      for seg in back-segs {
        line(..seg, stroke: (paint: pbc-lr.transparentize(60%), thickness: 1pt, dash: (3pt, 3pt)))
      }
      // Draw front segments on top (thick, bold)
      for seg in front-segs {
        line(..seg, stroke: (paint: pbc-lr, thickness: 1.8pt, dash: (5pt, 3pt)))
      }
    }

    // ── Purple poloidal ring (v=0, sweep u 0..360) ──
    {
      let n = 180
      let back-segs = ()
      let front-segs = ()
      ...
      // For the poloidal ring, visibility depends on whether the surface normal
      // at that point faces toward or away from the viewer.
      // Use the outward normal dot forward vector as visibility test.
      let polo-visible(u-deg) = {
        let u = u-deg * calc.pi / 180.0
        let v = 0.0 * calc.pi / 180.0
        let nx = calc.cos(u) * calc.cos(v)
        let ny = calc.cos(u) * calc.sin(v)
        let nz = calc.sin(u)
        let nxr = nx * cra - ny * sra
        let nyr = nx * sra + ny * cra
        let nzr = nz
        let dot = fwx * nxr + fwy * nyr + fwz * nzr
        dot < 0
      }

      for k in range(n + 1) {
        let u = float(k) / float(n) * 360.0
        let pt = torus-pt(u, 0.0)
        let is-front = polo-visible(u)
        if cur-front == none { cur-front = is-front }
        if is-front != cur-front {
          if cur-pts.len() > 1 {
            if cur-front { front-segs.push(cur-pts) } else { back-segs.push(cur-pts) }
      ...
      for seg in back-segs {
        line(..seg, stroke: (paint: pbc-tb.transparentize(60%), thickness: 1pt, dash: (3pt, 3pt)))
      }
      // Draw front segments on top
      for seg in front-segs {
        line(..seg, stroke: (paint: pbc-tb, thickness: 1.8pt, dash: (5pt, 3pt)))
      }
    }

    // ── Double-headed arrow between lattice and torus ──
    let arr-left  = 0.5
    let arr-right = tcx - 2.5
    line((arr-left, tcy), (arr-right, tcy),
      stroke: 3pt + arrow-gray,
      mark: (start: ">", end: ">", fill: arrow-gray, size: 0.35))

  }),
    )),

    // ══════════════════════════════════════════════════════════════
    //  PANEL (b): Gate building blocks  — VERBATIM from original
    // ══════════════════════════════════════════════════════════════
    figure(
      kind: "subfigure",
      caption: [],
    ...
    let faded-pos = (-6.3, -4.5)
    let bright-pos = (-3.7, -4.5)
    ...
    ball(faded-pos, "faded", "", sup: "1", size: 0.36)
    ball(bright-pos, "red", "", sup: "0", size: 0.36)

    // ============================================================
    // Bottom text
    // ============================================================
    content((0, -9.1), text(weight: "bold", size: 11pt)[No measurement, No movement, No local addressing])
    // content((0, -9.6), text(weight: "bold", size: 9pt)[— No local addressing])
  }),
    ),


    // ══════════════════════════════════════════════════════════════
    //  PANEL (d): Single-Plaquette Circuit  — from panel_d_circuit.typ
    // ══════════════════════════════════════════════════════════════
    grid.cell(colspan: 2, figure(
      kind: "subfigure",
      caption: [],
  ...
  // Z-error correction gate x-positions (shifted right for separation)
  let xh3  = 22.0   // Hadamard set 3 (ancilla only)
  let xor2 = 24.6   // OR gate 2
  let xcc2 = 27.2   // CCX syndrome 2
  let xh4  = 29.8   // Hadamard set 4 (ancilla only)
  let xcr2 = 32.8   // CCX correction 2
  let xrs2 = 35.3   // reset 2
  let xend = 37.0
  ...
  // Z-error: Syndrome Mapping (yellow, dashed)
  rect((xh3 - 1.0, yb - 1.0), (xh4 + 1.0, ya + 1.5),
    fill: syndrome-bg,
    stroke: (paint: orange-border, thickness: 1.5pt, dash: "dashed"),
    radius: 0.3)
  content(((xh3 + xh4) / 2, ya + 2.1),
    text(weight: "bold", size: 10pt, fill: rgb("#8a6500"))[Syndrome Mapping])

  // Z-error: Correction & Reset (green, dashed)
  rect((xcr2 - 1.2, yb - 1.0), (xrs2 + 1.1, ya + 1.5),
    fill: correct-bg,
    stroke: (paint: correct-bdr, thickness: 1.5pt, dash: "dashed"),
    radius: 0.3)
  content(((xcr2 + xrs2) / 2, ya + 2.1),
    text(weight: "bold", size: 10pt, fill: rgb("#2a6a2a"))[Correction & Reset])

  // ══════════════════════════════
  //  HORIZONTAL WIRES (separate for X-error and Z-error)
  // ══════════════════════════════
  let xzb = xh3 - 2.0   // Z-error circuit atom ball x-position

  // X-error wires
  for y in (ya, yd, yb) {
    line((xb + 0.5, y), (xrs + 1.0, y), stroke: 1pt + black)
  }
  // Z-error wires
  for y in (ya, yd, yb) {
    line((xzb + 0.5, y), (xend, y), stroke: 1pt + black)
  }
  ...
  // ══════════════════════════════
  //  Z-ERROR ATOM BALLS  (green ancillas, blue data)
  // ══════════════════════════════
  // a₁  (green, top)
  circle((xzb + 0.04, ya - 0.04), radius: br + 0.02, fill: shadow-c, stroke: none)
  circle((xzb, ya), radius: br,
    fill: gradient.radial(
      (green-spec, 0%), (green-mid, 50%), (green-drk, 100%),
      center: (30%, 25%), radius: 72%),
    stroke: 1.2pt + green-rim)
  content((xzb, ya),
    text(fill: white, weight: "bold", size: 7.5pt)[$a_1$])

  // D  (blue, middle)
  circle((xzb + 0.04, yd - 0.04), radius: br + 0.02, fill: shadow-c, stroke: none)
  circle((xzb, yd), radius: br,
    fill: gradient.radial(
      (blue-spec, 0%), (blue-mid, 50%), (blue-drk, 100%),
      center: (30%, 25%), radius: 72%),
    stroke: 1.2pt + blue-rim)
  ...
  // a₂  (green, bottom)
  circle((xzb + 0.04, yb - 0.04), radius: br + 0.02, fill: shadow-c, stroke: none)
  circle((xzb, yb), radius: br,
    fill: gradient.radial(
      (green-spec, 0%), (green-mid, 50%), (green-drk, 100%),
      center: (30%, 25%), radius: 72%),
    stroke: 1.2pt + green-rim)
  ...
  // ══════════════════════════════
  //  SECTION LABELS: X-error / Z-error
  // ══════════════════════════════
  content((xb, ya + 3.5), anchor: "west",
    text(weight: "bold", size: 10pt)[X-error correction])
  content((xzb, ya + 3.5), anchor: "west",
    text(weight: "bold", size: 10pt)[Z-error correction])

  // ══════════════════════════════════════════
  //  Z-ERROR CORRECTION CIRCUIT
  // ══════════════════════════════════════════

  // ── Hadamard gates set 3 (ancilla only) ──
  for y in (ya, yb) {
    rect((xh3 - hs, y - hs), (xh3 + hs, y + hs),
      fill: white, stroke: 1.2pt + black, radius: 0.06)
    content((xh3, y),
      text(weight: "bold", size: 10pt)[H])
  }

  // ── OR gate 2 (orange box) ──
  rect((xor2 - hw, yb - gpad), (xor2 + hw, ya + gpad),
    fill: orange-bg, stroke: 2pt + orange-border, radius: 0.2)
  content((xor2, yd),
    text(weight: "bold", size: 11pt)[OR])

  // ── CCX syndrome gate 2 (teal box) ──
  rect((xcc2 - hw, yb - gpad), (xcc2 + hw, ya + gpad),
    fill: teal-bg, stroke: 2pt + teal-border, radius: 0.2)
  content((xcc2, yd),
    text(weight: "bold", size: 10pt)[CCX])

  // ── Hadamard gates set 4 (ancilla only) ──
  for y in (ya, yb) {
    rect((xh4 - hs, y - hs), (xh4 + hs, y + hs),
      fill: white, stroke: 1.2pt + black, radius: 0.06)
    content((xh4, y),
      text(weight: "bold", size: 10pt)[H])
  }

  // ── CCX correction gate 2 (dark red) ──
  rect((xcr2 - hw, yb - gpad), (xcr2 + hw, ya + gpad),
    fill: darkred-bg, stroke: 2pt + darkred, radius: 0.2)
  content((xcr2, yd),
    text(weight: "bold", size: 10pt, fill: darkred)[CCX])
  ...
  // ── Reset |0⟩ boxes 2 ──
  for y in (ya, yb) {
    rect((xrs2 - 0.6, y - hs), (xrs2 + 0.6, y + hs),
      fill: white, stroke: 1.2pt + black, radius: 0.06)
    content((xrs2, y),
      text(size: 9pt)[$|0 angle.r$])
  }
  ...
  // ══════════════════════════════
  //  ANNOTATION: OR + CCX = XOR (Z-error)
  // ══════════════════════════════
  let by2 = yb - 1.5
  line((xor2 - hw, by2 + 0.4), (xor2 - hw, by2), stroke: 0.8pt + arrow-gray)
  line((xor2 - hw, by2), (xcc2 + hw, by2), stroke: 0.8pt + arrow-gray)
  line((xcc2 + hw, by2 + 0.4), (xcc2 + hw, by2), stroke: 0.8pt + arrow-gray)
  line(((xor2 + xcc2) / 2, by2), ((xor2 + xcc2) / 2, by2 - 0.25),
    stroke: 0.8pt + arrow-gray)
  content(((xor2 + xcc2) / 2, by2 - 0.65),
    text(size: 8.5pt, fill: arrow-gray, style: "italic")[OR + CCX = parity (XOR)])
  }),
    )),
