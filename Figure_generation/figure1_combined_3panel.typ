...
#figure(
  grid(
    ...
    grid.cell(colspan: 2, figure(
      kind: "subfigure",
      caption: [],
      box(width: 100%, move(dx: -1.25cm, cetz.canvas(length: 1.1cm, {
    import cetz.draw: *
    ...
    // ══════════════════════════════════════════
    //  GRID LINES
    //  Edges touching a PBC boundary-copy vertex
    //  (i == 0 row or j == nv-1 column) are dashed.
    // ══════════════════════════════════════════
    for i in range(nv) {
      for j in range(nv - 1) {
        ...
        // horizontal edge endpoints: (j, i) and (j+1, i)
        let is-pbc-edge = (i == 0) or (j + 1 == nv - 1)
        if is-pbc-edge {
          line((x1, y1), (x2, y2),
            stroke: (paint: line-color, thickness: 1.6pt, dash: (5pt, 3pt)))
        } else {
    ...
    for j in range(nv) {
      for i in range(nv - 1) {
        ...
        // vertical edge endpoints: (j, i) and (j, i+1)
        let is-pbc-edge = (j == nv - 1) or (i == 0)
        if is-pbc-edge {
          line((x1, y1), (x2, y2),
            stroke: (paint: line-color, thickness: 1.6pt, dash: (5pt, 3pt)))
        } else {
    ...
    // === Title ===
    content((-4.4 + 0.6, 0.1), text(weight: "bold", size: 15pt)[QEC Cycle])
    content((-7.0 + 0.6, 0.1), text(weight: "bold", size: 15pt)[(b)])
    ...
    ball((a1x, a1y), "faded", "A", label-color: white, size: 0.36)
    ball((a2x, a2y), "faded", "A", label-color: white, size: 0.36)
    ...
    // ══════════════════════════════════════════════════════════════
    //  PANEL (c): Gate building blocks  — VERBATIM from original
    // ══════════════════════════════════════════════════════════════
    figure(
      kind: "subfigure",
      caption: [],
    ...
    //  TITLE
    // ══════════════════════════════════════════
    content((0.3 + 0.6, 10.5), anchor: "west",
      text(weight: "bold", size: 15pt)[(c)])
    content((1.6 + 0.6, 10.5), anchor: "west",
      text(weight: "bold", size: 15pt)[Three-qubit Building Blocks.])

    // ══════════════════════════════════════════
    //  LEFT PANEL  –  OR Gate
    // ══════════════════════════════════════════
    rect((lx0, py0 - 0.6), (lx0 + pw, py1),
      radius: 0.3, stroke: 2.5pt + orange-border, fill: orange-bg)

    content((lcx, py1 - 0.50), anchor: "center",
      text(weight: "bold", size: 10pt)[OR-Toffoli])
    ...
    content((cs-x, cs-0y - 0.85), anchor: "center",
      text(weight: "bold", size: 9pt)[Ancilla])
    
    content((cs-x, cs-0y - 1.6), anchor: "center",
      text(weight: "bold", size: 9pt)[(Control)])
    ...
    // |P⟩
    line((rb-cx - 0.5, rb-Py + 0.5), (rb-cx + 0.5, rb-Py + 0.5), stroke: lev-stroke + black)
    content((rb-cx + 0.65, rb-Py), anchor: "west",
      text(size: 9pt)[$|P angle.r$])
    // |P⟩ detuning Δ: dashed virtual photon level above |P⟩, split around
    // the central Ω_R arrow; Δ marker on the left to avoid the |P⟩ label.
    let rb-Pdy = rb-Py 
    line((rb-cx - 0.5, rb-Pdy), (rb-cx - 0.10, rb-Pdy),
      stroke: (paint: gray, thickness: 1pt, dash: (3pt, 2pt)))
    line((rb-cx + 0.10, rb-Pdy), (rb-cx + 0.5, rb-Pdy),
      stroke: (paint: gray, thickness: 1pt, dash: (3pt, 2pt)))
    line((rb-cx - 0.40, rb-Py + 0.05), (rb-cx - 0.40, rb-Pdy - 0.05),)
    content((rb-cx - 0.50, (rb-Py + rb-Pdy) / 2 + 0.2), anchor: "east",
      text(size: 8pt)[$Delta$])
    // |R⟩
    ...
    // Ω_R : |P⟩ ↔ |R⟩
    line((rb-cx, rb-Py + 0.5), (rb-cx, rb-Ry),
      stroke: 1.3pt + yellow-arrow,
      mark: (start: "straight", end: "straight", fill: none, stroke: yellow-arrow, size: 0.18))
    content((rb-cx - 0.25, (rb-Py + rb-Ry) / 2 + 0.2), anchor: "east",
      text(fill: yellow-arrow, size: 8pt)[$Omega_R$])

    content((rb-cx, rb-Ay - 0.85), anchor: "center",
      text(weight: "bold", size: 9pt)[Data])

    content((rb-cx, rb-Ay - 1.6), anchor: "center",
      text(weight: "bold", size: 9pt)[(Target)])

    // ── Bottom text ──
    content((lcx, py0 + 0.1), anchor: "center",
      text(size: 9pt)[Target flips if any Control = $|1 angle.r$])

    // ══════════════════════════════════════════
    //  RIGHT PANEL  –  CCX Gate
    // ══════════════════════════════════════════
    rect((rx0, py0 - 0.6), (rx0 + pw, py1),
    ...
    content((cs2-x, cs-0y - 0.85), anchor: "center",
      text(weight: "bold", size: 9pt)[Ancilla])
    
    content((cs2-x, cs-0y - 1.6), anchor: "center",
      text(weight: "bold", size: 9pt)[(Control)])
    ...
    content((rb2-cx, rb-Ay - 0.85), anchor: "center",
      text(weight: "bold", size: 9pt)[Data])
    
    content((rb2-cx, rb-Ay - 1.6), anchor: "center",
      text(weight: "bold", size: 9pt)[(Target)])

    // ── Bottom text ──
    content((rcx, py0 + 0.1), anchor: "center",
